...
class LoadData:
    ...
    def __init__(self, val_size: int = 100):
        ...
        self.pool_size = 50000 - self.train_size - self.val_size        # comment out if use MNIST
        # self.pool_size = 60000 - self.train_size - self.val_size
        self.cifar10_train, self.cifar10_test = self.download_dataset() # comment out if use MNIST
    ...
    def check_CIFAR10_folder(self) -> bool:
        """Check whether CIFAR10 folder exists, skip download if existed"""
        if os.path.exists("CIFAR10/"):
            return False
        return True

    def download_dataset(self):
        """Load MNIST/CIFAR10 dataset for training and test set."""
        transform = transforms.Compose(
            [transforms.ToTensor(), transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010)),])
        
        download = self.check_CIFAR10_folder()
        cifar10_train = CIFAR10(".", train=True, download=download, transform=transform)
        cifar10_test = CIFAR10(".", train=False, download=download, transform=transform)
    ...
    def split_and_load_dataset(self):
        ...
        train_set, val_set, pool_set = random_split(
            self.cifar10_train, [self.train_size, self.val_size, self.pool_size]
        ...
        test_loader = DataLoader(
          dataset=self.cifar10_test, batch_size=10000, shuffle=True
